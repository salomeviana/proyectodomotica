...
def verificacion_clave():
    ...
    global col
    if _clave == "micasa":
        verify.config(text="HABILITADO")
        col="green"
        print("El sistema de alarmas ha sido habilitado")
    else:
        verify.config(text="INCORRECTA")
        col="red"
...
verify.config(bg=f_color, fg="red", font=fuente, cursor=typ_c,width=10, justify="center",text="")


#----creacion botones ventilacion----------------------------

vent_on=Button(text=on,bg=f_color, font=fuente, cursor=typ_c, activebackground=fc_color,activeforeground=lc_color)
vent_on.place(x=1120,y=570)
vent_off= Button(text=off,bg=f_color, font=fuente,cursor=typ_c, activebackground=fc_color,activeforeground=lc_color)
